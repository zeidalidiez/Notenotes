...
Plus: **16 synth presets** split into Chip and Modern families, picked from the same searchable library pattern as scales. Chip stays punchy and direct; Modern is voiced fuller with supporting oscillators, light unison, filter movement, key tracking, and restrained patch drive. **Tone** controls shape sound without becoming a full DAW, and the entire instrument layer is [pluggable](#-build-your-own-instrument).
...
- **Create: playing and capturing.** 16 synth presets, 4 drum kits, scale-locked or chromatic, hold and arpeggio modes. Chip, Modern, and Custom patch instruments are chosen from a searchable picker so the list can grow without turning into a scroll trap. Chip presets stay punchy and retro, while every Modern preset uses richer synth motion like a supporting oscillator, light unison, filter movement, vibrato or key tracking, and restrained drive without making quick taps disappear.
